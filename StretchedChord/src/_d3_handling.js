...
export function drawDiagram(cord) {
    ...
    function link(link) {
        let q1 = [2 * cord._width * Math.sin(link.source.startAngle) / 5, -2 * cord._width * Math.cos(link.source.startAngle) / 5],
            q2 = [9 * cord._width * Math.sin(link.source.startAngle) / 20, -9 * cord._width * Math.cos(link.source.startAngle) / 20],
            q3 = [9 * cord._width * Math.sin(link.source.endAngle) / 20, -9 * cord._width * Math.cos(link.source.endAngle) / 20],
            q4 = [2 * cord._width * Math.sin(link.source.endAngle) / 5, -2 * cord._width * Math.cos(link.source.endAngle) / 5],
            q5 = [2 * cord._width * Math.sin(link.target.endAngle) / 5, -2 * cord._width * Math.cos(link.target.endAngle) / 5],
            q6 = [9 * cord._width * Math.sin(link.target.endAngle) / 20, -9 * cord._width * Math.cos(link.target.endAngle) / 20],
            q7 = [9 * cord._width * Math.sin(link.target.startAngle) / 20, -9 * cord._width * Math.cos(link.target.startAngle) / 20],
            q8 = [2 * cord._width * Math.sin(link.target.startAngle) / 5, -2 * cord._width * Math.cos(link.target.startAngle) / 5];
    ...
    function addLHSlabel() {
        ...
        let label = d3.select('#L').append('text')
    ...
    function addRHSlabels() {
        ...
        function addLabel(path) {
            let offset = getOffset(),
                formattedLabel = getLabelFormatted(path.attr('name'), Math.abs(offset[0])),
                label = d3.select('#R').append('text')
            ...
            function getOffset() {
                let center = d3.arc().innerRadius(cord._width / 2).outerRadius(cord._width / 2).centroid(path.datum());
...
export function createGradients(cord) {
    cord._links.forEach(function (d) {
        let col = d.criticality.slice(1), dir = d.source.id === cord._LHSnode.id ? 'r' : 'l';
...
function linkMouseover(cord) {
    return function (d) {
        let center = - (Math.cos(d.source.endAngle) + Math.cos(d.target.endAngle) + Math.cos(d.source.startAngle) + Math.cos(d.target.startAngle)) * 0.07125 * cord._width;
        ...
        let node = d3.select('#bandwidth').node().getBBox()
